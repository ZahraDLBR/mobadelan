...
class SimpleTest(unittest.TestCase):
    ...
    def test_register_exam(self):
        for i in range (0 , 2):
            self.increase_wallet(self.json["exam"][i])
            self.driver.find_element_by_id("paymentBtn").click()
            assert "payment" in self.driver.title
            self.driver.find_element_by_id("examBtn").click()
            select = Select(self.driver.find_element_by_id('pay4dropdown'))
            select.select_by_index(i)
            self.driver.find_element_by_id("submitBtn").click()
            #commission
            assert "commission" in self.driver.title
            assert self.driver.find_element_by_id("commission").text is self.json["commission"]["exam"][i]
