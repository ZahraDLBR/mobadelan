...
class SimpleTest(unittest.TestCase):
    ...
    def test_good(self):
        for i in range (0 , len(self.json["university"])):
            self.increase_wallet(self.json["university"][i])
            self.driver.find_element_by_id("paymentBtn").click()
            assert "payment" in self.driver.title
            self.driver.find_element_by_id("tuitionBtn").click()
            select = Select(self.driver.find_element_by_id('pay2dropdown'))
            select.select_by_index(i)
            self.driver.find_element_by_id("pay2varizi").send_keys("20000")
            self.driver.find_element_by_id("submitBtn").click()
            #commission
            assert "commission" in self.driver.title
            assert self.driver.find_element_by_id("commission").text is self.json["commission"]["university"][i]
